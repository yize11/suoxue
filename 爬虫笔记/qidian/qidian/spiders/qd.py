...
from qidian.items import QidianNovalItem
...
class QdSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for noval_li in noval_lis:
            ...
            # 封面
            novalItem['coverImage'] = 'https:' + noval_li.xpath(
            ...
            detailUrl = 'https:' + noval_li.xpath('.//div[@class="book-mid-info"]/h4/a/@href').extract_first(
                '') + '#Catalog'
            ...
            for div in div_volume:
                ...
                if '免费' in isFree:
                    ...
                    for chpater_li in chpater_lis:
                        ...
                        yield scrapy.Request(url=chpater_url, callback='')
        else:
    ...
    def parse_chpater_detail(self, response):
        ...
        """
        qdInfo = {}
        qdInfo['title'] = response.xpath('//h3[@class="j_chapterName"]/text()').extract_first('')
        print(qdInfo)
